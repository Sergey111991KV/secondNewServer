...
deleteOldSession :: PG r m => User -> m Int64
deleteOldSession us = do
  withConn $ \conn -> execute conn qry [id_user us]
  where
    qry = "delete from session where user_id = ?"

insertNewSession :: PG r m => User -> m Int64
insertNewSession us = do
  sId <- liftIO $ stringRandomIO "[a-zA-Z0-9]{32}"
  withConn $ \conn -> execute conn qry (sId, id_user us)
  where
  ...
  case i of
    Right uIdStr -> do
      ...
      case i of
        ...
        [] -> do
          Log.logIn Log.Error $
            "Error findUserBySession " ++ errorText DataErrorPostgreSQL -- log
          return $ Left DataErrorPostgreSQL
