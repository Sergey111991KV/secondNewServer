...
getOne sess text idE
  | text == "news" = do
    ...
    case i of
      ...
      [] -> do
        Log.logIn Log.Error $
          "Error getOne news" ++ errorText DataErrorPostgreSQL
        ...
        if authAdmin user
          then do
            ...
            case i of
              ...
              [] -> do
                Log.logIn Log.Error $
                  "Error getOne author" ++ errorText DataErrorPostgreSQL
                return $ Left DataErrorPostgreSQL
          else do
            Log.logIn Log.Error $
              "Error getOne author" ++ errorText AccessErrorAdmin
            return $ Left AccessErrorAdmin
      Left err -> do
        Log.logIn Log.Error $
          "Error getOne author" ++ errorText UserErrorFindBySession
        return $ Left UserErrorFindBySession
  | text == "category1" = do
    let q =
          "SELECT category_1.id_c1, category_1.description_cat1  FROM category_1 where category_1.id_c1 = (?)"
    ...
    case i of
      ...
      [] -> do
        Log.logIn Log.Error $
          "Error getOne category1" ++ errorText DataErrorPostgreSQL
        return $ Left DataErrorPostgreSQL
  | text == "category2" = do
    let q =
          "SELECT category_2.id_c2, category_2.description_cat2, category_1.id_c1, category_1.description_cat1  FROM category_2, category_1 where category_2.id_c2 = (?) AND category_2.category_1_id = category_1.id_c1"
    ...
    case i of
      ...
      [] -> do
        Log.logIn Log.Error $
          "Error getOne category2" ++ errorText DataErrorPostgreSQL
        return $ Left DataErrorPostgreSQL
  | text == "category3" = do
    let q =
          "SELECT category_3.id_c3, category_3.description_cat3, category_2.id_c2, category_2.description_cat2, category_1.id_c1, category_1.description_cat1  FROM category_3, category_2, category_1 where category_3.id_c3 = (?) AND category_2.category_1_id = category_1.id_c1 AND category_3.category_2_id = category_2.id_c2"
    ...
    case i of
      ...
      [] -> do
        Log.logIn Log.Error $
          "Error getOne category3" ++ errorText DataErrorPostgreSQL
        return $ Left DataErrorPostgreSQL
  | text == "draft" = do
    access <- findUserBySession sess
    case access of
        ...
        if authAuthor user
          then do
            ...
            case result of
              ...
              [] -> do
                Log.logIn Log.Error $
                  "Error getOne draft" ++ errorText DataErrorPostgreSQL
                return $ Left DataErrorPostgreSQL
          else do
            Log.logIn Log.Error $
              "Error getOne draft" ++ errorText AccessErrorAuthor
            return $ Left AccessErrorAuthor
      Left err -> do
        Log.logIn Log.Error $
          "Error getOne draft" ++ errorText UserErrorFindBySession
        return $ Left UserErrorFindBySession
  | text == "comment" = do
    let q =
          "SELECT   (element_comment).id_comments  \
    ...
    case i of
      ...
      [] -> do
        Log.logIn Log.Error $
          "Error getOne comment" ++ errorText DataErrorPostgreSQL
        return $ Left DataErrorPostgreSQL
  | text == "tag" = do
    let q =
          "SELECT   (element_tags).id_teg  \
    ...
    case i of
      ...
      [] -> do
        Log.logIn Log.Error $
          "Error getOne tag" ++ errorText DataErrorPostgreSQL
        return $ Left DataErrorPostgreSQL
  | text == "user" = do
    ...
    case i of
      ...
      [] -> do
        Log.logIn Log.Error $
          "Error getOne user" ++ errorText DataErrorPostgreSQL
        return $ Left DataErrorPostgreSQL
