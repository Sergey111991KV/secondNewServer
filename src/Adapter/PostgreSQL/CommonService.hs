...
--                                 []      -> Left DataErrorPostgreSQL
                    ...
                    | text == "teg" = do
                        let q = "SELECT * FROM tags where id = (?)"
...
-- convertToAuthor (Author' idA text idU) = do
--         user <- getOne True "user" idU
--         return $ case user of
--             Left err ->  Left err
--             Right entUser ->  Right $ convertToEntity (Author idA text (convertFromEntity entUser))


-- convertToCategoryPostgress :: PG r m =>  Category' -> m (Either Error Entity)
-- convertToCategoryPostgress = undefined
-- -- -- convertToCategoryPostgress (CatCategory1' (Category1 idC nameC)) = do 
-- -- convertToCategoryPostgress (CatCategory3' (Category3' idC nameC id2)) = do 
-- --         let q = "SELECT * FROM category_2 where id = (?)"
-- --         i <- (withConn $ \conn -> query conn q [idE] :: IO [Category2']) 
-- --         case i of
-- --                 []      -> return $ Left DataErrorPostgreSQL
